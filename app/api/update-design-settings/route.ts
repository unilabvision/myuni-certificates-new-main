  ...
  try {
    ...
    if (!organizationSlug) {
      return NextResponse.json({
        success: false,
        error: 'organizationSlug gerekli'
      }, { status: 400 });
    }
    
    console.log(`${organizationSlug} için design settings güncelleniyor...`);
    
    // Yeni design settings
    ...
    console.log(`${organizationSlug} template güncellendi:`, data);
    
    return NextResponse.json({
      success: true,
      message: `${organizationSlug} template güncellendi`,
      template: {
        id: data.id,
        name: data.name,
    ...
    console.error('Design settings güncelleme hatası:', error);
    return NextResponse.json({
      success: false,
